...
def serialize(
    ...
    left = ghcc.parse.visit_dict(
        find_min_pos_fn, ast_dict[ghcc.parse.CHILDREN_ATTR]["decl"]
    )
...
def match_functions(
    ...
    *,
    ...
    if progress_bar is not None:
        ...
        process_name = (
            f"Worker {worker_id}" if worker_id is not None else "Main Process"
        )
    ...
    for makefile in makefiles:
        ...
        for path, sha in zip(makefile["binaries"], makefile["sha256"]):
            ...
            try:
                original_ast: ASTNode = parser.parse(
            ...
            try:
                ...
                for func_name, (begin, end) in func_begin_end.items():
                    if (
                        begin is not None
                        and end is not None
                        and func_name in function_asts
                    ):
...
def _iter_repos(
    ...
    with contextlib.closing(ghcc.RepoDB()) as repo_db, contextlib.closing(
        ghcc.BinaryDB()
    ) as binary_db:
        ...
        def _get_repo_binaries_info() -> Dict[Tuple[str, str], Set[str]]:
            ...
            for entry in tqdm.tqdm(
                all_bins, total=all_bins.count(), desc="Listing decompiled binaries"
            ):
                if entry["success"]:
                    repo_binaries[entry["repo_owner"], entry["repo_name"]].add(
                        entry["sha"]
                    )
            return repo_binaries

        repo_binaries = _get_repo_binaries_info()
        repo_entries: Iterator[ghcc.RepoDB.Entry] = repo_db.safe_iter(
            batch_size=10000, static=True
        )
        if skip_to is not None:
            skip_to_repo = tuple(skip_to.split("/"))
            repo_entries = flutes.drop_until(
                lambda entry: (entry["repo_owner"], entry["repo_name"]) == skip_to_repo,
                repo_entries,
            )
        index = 0
        for entry in repo_entries:
            if (
                not entry["clone_successful"]
                or not entry["compiled"]
                or len(entry["makefiles"]) == 0
                or entry["num_binaries"] == 0
            ):
                # Nothing to match.
                continue
...
) -> Tuple[Iterator[RepoInfo], DBStats]:
        ...
        for result in pool.imap_unordered(match_fn, iterator):
            ...
            with (repo_dir / "matched_funcs.jsonl").open("w") as f:
                for matched_func in result.matched_functions:
                    f.write(
                        json.dumps(matched_func._asdict(), separators=(",", ":")) + "\n"
                    )
